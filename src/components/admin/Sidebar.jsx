...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  const links = [
    ...
    { to: "/admin/chat", label: "Contacts", icon: <FiMail /> },
  ...
  return (
    <aside
       className={`sidebar m-0 shadow-lg ${isOpen ? "show" : ""}`}
            style={{
                width: isMobile ? (isOpen ? "240px" : "") : isExpanded ? "240px" : "",
                transition: "width 0.3s",
            }} 
      /* className={`sidebar m-0 shadow-lg ${
    ...
    >
      ...
      <div className="sidebar-header p-3 border-bottom d-flex align-items-center">
        ...
        {(isOpen || isExpanded) && (
          <span className="ms-2 fw-bold">Admin Panel</span>
        )}
      </div>
          ...
          {links.map((link) => (
            <li key={link.to}>
              <NavLink
                to={link.to}
                className={({ isActive }) =>
                  `sidebar-link d-flex align-items-center gap-2 px-2 py-2 rounded-3 ${
