...
const CartItem = ({ product }) => {
  ...
  const [selectedSize, setSelectedSize] = useState(
    product.selectedSize || product.sizes?.[0] || "Free Size"
  );

  // Handle size change
  const handleSizeChange = (size) => {
    setSelectedSize(size);
    updateProductSize(product.id, product.selectedSize, size);
  };

  // Current price based on size
  const currentPrice = useMemo(() => {
    const basePrice = product.priceBySize?.[selectedSize] || product.price || 0;
    const discount = product.discount || 0;
    return basePrice - (basePrice * discount) / 100;
  }, [selectedSize, product.priceBySize, product.price, product.discount]);

  // Format price for display in local currency
  ...
  const formattedTotal = new Intl.NumberFormat(navigator.language, {
    style: "currency",
    currency: product.currency || "INR",
  }).format(currentPrice * product.quantity);
  ...
  const moveToCheckout = () => {
    navigate("/checkout", {
      state: { itemsToCheckout: [{ ...product, selectedSize }] },
    });
    removeFromCart(product.id, selectedSize);
  };

  return (
    <div className="cart-item border d-flex col mb-2">
      {/* Product Image */}
      ...
      <div className="product-details col-6 p-3">
        ...
        <span className="product-price h5">
      ...
      <div className="price text-end mt-3 ms-4 col-2 d-flex flex-column">
        ...
        {product.discount > 0 && <div>{product.discount}% OFF</div>}
