...
function Dashboard() {
  ...
  const filterByDate = (items, dateKey, timeframe, offset = 0) => {
    ...
    return items.filter((item) => {
      const raw =
        item[dateKey] ?? item.createdAt ?? item.date ?? item.registeredAt;
      if (!raw) return false;
  ...
  const calculateStats = () => {
    ...
    const filteredReviews = filterByDate(
      reviewsData,
      "createdAt",
      filters.reviews
    );
    ...
    const previousOrders = filterByDate(
      ordersData,
      "orderDate",
      filters.orders,
      1
    ).length;
    const previousUsers = filterByDate(
      usersData,
      "registeredAt",
      filters.users,
      1
    ).length;
    const previousProducts = filterByDate(
      productsData,
      "createdAt",
      filters.products,
      1
    ).length;
    const previousReviews = filterByDate(
      reviewsData,
      "createdAt",
      filters.reviews,
      1
    ).length;
    const previousRevenue = filterByDate(
      ordersData,
      "orderDate",
      filters.orders,
      1
    ...
    setChanges({
      ...
      avgOrderValue: calcChange(
        avgOrderValue,
        previousRevenue / Math.max(1, previousOrders)
      ),
    });
  };
  ...
  const salesSeries = useMemo(() => {
    ...
    ordersData.forEach((o) => {
      ...
      if (chartMetric === "AOV")
        value = getOrderAmount(o) / (o.items?.length ?? 1);
      bucket.amount += value;
    });
  ...
  const bestSellingProducts = useMemo(() => {
    ...
    ordersData.forEach((o) => {
      ...
      items.forEach((it) => {
        ...
        if (!pid) return;
        const price =
          it.price ?? productsData.find((p) => p.id === pid)?.price ?? 0;
        const qty = it.quantity ?? 1;
  ...
  const recentOrders = useMemo(() => {
    return [...ordersData]
      .sort(
        (a, b) =>
          new Date(b.orderDate ?? b.createdAt ?? b.timestamp) -
          new Date(a.orderDate ?? a.createdAt ?? a.timestamp)
      )
      .slice(0, 12)
      .map((o) => ({
        id: o.orderId ?? `#${Math.random().toString(36).slice(2, 8)}`,
        date: new Date(
          o.orderDate ?? o.createdAt ?? o.timestamp
        ).toLocaleString(),
  ...
  const bestSellingColumns = [
    {
      key: "image",
      label: "Image",
      render: (row) => <img src={row.image} alt={row.name} width={40} />,
    },
  ...
  return (
    <div className={`m-0 dashboard-wrapper ${theme}`}>
      {/* Header */}
        ...
        <div>
          ...
          <button
            className={`btn btn-sm ${
              live ? "btn-success" : "btn-outline-secondary"
            } mt-1`}
          ...
          <button
            ...
            onClick={() => {
              if (navigator.share) {
                ...
                showToast(
                  "Sharing is not supported in your browser. Copied URL to clipboard!",
                  "info"
                );
      ...
      <div className="row g-3 mb-4">
        {[
          ...
          <div className="col-md-4" key={stat.key}>
            <div
              className={`card stat-card p-3 shadow-sm h-100 position-relative ${theme} ${stat.key}`}
            >
              <div className="d-flex justify-content-between align-items-start">
                <div>
                  <h6 className="mb-1 stat-label">{stat.label}</h6>
                  <h3 className="stat-value">
                    {stat.key === "revenue" || stat.key === "avgOrderValue"
                      ? `₹ ${stats[stat.key].toLocaleString()}`
                      : stats[stat.key]}
                  </h3>
                  <p
                    className={`mb-0 stat-change small ${
                      changes[stat.key] > 0
                        ? "text-success"
                        : changes[stat.key] < 0
                        ? "text-danger"
                        ...
                        <li key={period}>
                          <button
                            className={`dropdown-item text-capitalize ${
                              filters[stat.key] === period ? "active" : ""
                            }`}
                            onClick={() =>
                              setFilters((prev) => ({
                                ...prev,
              ...
              <div className="mt-3 d-flex justify-content-end">
                <div
                  className="stat-icon bg-gradient d-flex justify-content-center align-items-center rounded-circle"
                  style={{
                    width: "40px",
                    height: "40px",
      ...
      <div className="btn-group mb-3 flex-wrap mt-2">
        {["Revenue", "Orders", "AOV"].map((metric) => (
          <button
            key={metric}
            className={`btn btn-sm ${
              chartMetric === metric ? "btn-primary" : "btn-outline-secondary"
            }`}
      ...
      <div className="row g-3 mb-4">
        <div className="col-lg-8">
          <div className={`card p-3 shadow-sm h-100 ${theme}`}>
            <div className="d-flex justify-content-between align-items-center mb-2">
              ...
              <div className="small ">Period: {cap(filters.orders)}</div>
            </div>
        ...
        <div className="col-lg-4">
          <div
            className={`card p-3 shadow-sm mb-3 overflow-auto ${theme}`}
              ...
              {usersData.slice(0, 8).map((u) => (
                ...
                >
                  ...
                  <div className="flex-grow-1 small">
                    ...
                    <div className=" small">
                      {new Date(
                        u.registeredAt ?? u.date ?? u.createdAt
                      ).toLocaleString()}
                    </div>
                  </div>
                </li>
          ...
          <div
            className={`card p-3 shadow-sm overflow-auto ${theme}`}
            style={{ maxHeight: "220px" }}
          >
                  ...
                  <div className="d-flex justify-content-between">
                    <div>
                      <strong className="small">
                        {productsData.find((p) => p.id === r.productId)?.name ??
                          "Unknown Product"}
                      </strong>
                      <div className="small ">
                        {r.text.slice(0, 80)}
