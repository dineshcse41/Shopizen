...
const CheckoutPage = () => {
  ...
  // Format any amount according to currency
  ...
  // Total price considering size-specific pricing
  const totalPrice = itemsToCheckout.reduce((sum, p) => {
    const sizePrice = p.priceBySize?.[p.selectedSize] ?? p.price;
    return sum + sizePrice * p.quantity;
  }, 0);
    ...
    const fetchDeliveryCharge = async () => {
      try {
        ...
        const response = await axios.post(
          "http://127.0.0.1:8000/api/delivery-charge/",
          {
  ...
  return (
    <>
      ...
      <div className="checkout-page d-flex flex-column flex-md-row gap-3">
        {/* LEFT SIDE FORM */}
        <div className="checkout-form flex-fill w-100 w-md-70 mt-3 mx-auto">
          <CheckoutForm
            user={user}
        ...
        {/* RIGHT SIDE SUMMARY */}
        <div className="checkout-summary flex-fill mt-3 me-2">
          <div className="total-sec p-3 border d-flex flex-column rounded shadow-sm">
            ...
            <div className="d-flex justify-content-between">
              ...
              <span>
                <strong>
                  {formatPrice(totalPrice, itemsToCheckout[0]?.currency)}
                </strong>
              </span>
            </div>
            ...
            <div className="d-flex justify-content-between">
              <span>Delivery:</span>
              <span>
                {formatPrice(deliveryCharge, itemsToCheckout[0]?.currency)}
              </span>
            </div>
            ...
            <div className="d-flex justify-content-between">
              ...
              <span className="h5">
                <strong>
                  {formatPrice(orderTotal, itemsToCheckout[0]?.currency)}
                </strong>
              </span>
            </div>
