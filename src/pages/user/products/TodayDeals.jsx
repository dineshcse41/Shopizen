...
const TodayDeals = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // Uncomment when backend fully ready
      // ================================
      ...
      // ================================
      // ⭐ JSON FALLBACK (Currently active)
      // ================================
     /*  setProducts(productsData); */

    } catch (err) {
      console.error("Error fetching products:", err);
      setError("Unable to load products. Showing backup data.");
  ...
  const filteredProducts = useMemo(() => {
    let result = products.filter((p) => p.dealOfTheDay);
  ...
  // =========================
  // ⭐ LOADING SCREEN (Amazon-like)
  // =========================
  if (loading) {
    return (
      <div className="text-center mt-5" style={{height: "167px"}}>
        <div className="spinner-border text-primary"></div>
        <p className="mt-3">Loading Today’s Deals…</p>
      </div>
    );
  }
    ...
    <>
      <div className="content-wrapper">
        ...
        <div className="products-container">
          {error && (
            <div className="alert alert-warning text-center mb-3">{error}</div>
          )}
